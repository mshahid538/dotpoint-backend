...
var allowlist = ["http://localhost:3001", "https://localhost:3001", "http://localhost:3000"];
...
const health = (req, res) => {
    return res.status(200).json({
        message: `Dot Point App ${
            process.env.NODE_ENV?.charAt(0)?.toUpperCase() + process.env.NODE_ENV?.slice(1)
        } Node.js (18.x) Server is Running, Server health is green`,
...
const bad_gateway = (req, res) => {
    return res.status(502).json({ status: 502, message: "Dot Point App Backend API Bad Gateway" });
};
